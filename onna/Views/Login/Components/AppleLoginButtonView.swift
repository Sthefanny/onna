                        ...
                        case let appleIDCredential as ASAuthorizationAppleIDCredential:
                            ...
                            if var firstName = appleIDCredential.fullName?.givenName,
                               ...
                               let email = appleIDCredential.email{
                                ...
                                let user = User(id: nil, image: getRandomProfileImage(), firstName: firstName, lastName: lastName, email: email, password: userId, birthDate: nil, insta: nil, phrase: nil, deviceId: "", version: "1.0.0", so: "iOS", token: nil)
                                viewModel.user = user
                                viewModel.createAndLoginUser { isLogged in
                                    ...
                                    if (isLogged){
                                        ...
                                        UserDefaults.standard.set(viewModel.user?.id, forKey: UserDefaultsKeys.idUsuario.name)
                                        self.action()
                                    } else {
                                        login(user: user)
                                    }
                                ...
                                publicDatabase.fetch(withRecordID: CKRecord.ID(recordName: userId)) { (record, error) in
                                    if let fetchedInfo = record {
                                        ...
                                        //Login user in API
                                        let user = User(id: nil, image: getRandomProfileImage(), firstName: nil, lastName: nil, email: email!, password: userId, birthDate: nil, insta: nil, phrase: nil, deviceId: nil, version: nil, so: nil, token: nil)
                                        login(user: user)
                                    }
                                }
                            }
